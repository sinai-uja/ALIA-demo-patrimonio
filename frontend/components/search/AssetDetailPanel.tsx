...
const AssetLocationMap = dynamic(() => import("./AssetLocationMap"), {
  ssr: false,
  loading: () => (
...
/** Returns true if a string value is non-null and non-whitespace. */
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <h4 className="text-xs font-semibold text-stone-400 uppercase tracking-wide">{title}</h4>
  ...
  return (
    <Section title="Tipologias">
      <div className="space-y-1.5">
        {valid.map((t, i) => (
          <div key={i} className="text-xs text-stone-600 border-l-2 border-stone-200 pl-2">
            {hasValue(t.typology) && <p className="font-medium text-stone-700">{t.typology}</p>}
  ...
  return (
    <Section title="Bibliografia">
      <ul className="space-y-2">
        {valid.map((b, i) => (
          <li key={i} className="text-xs text-stone-600 border-l-2 border-stone-200 pl-2">
            {hasValue(b.author) && <span className="font-medium">{b.author} </span>}
...
function InmaterialContent({ d, mapSlot }: { d: InmaterialDetails; mapSlot: React.ReactNode }) {
  const hasGeneral = hasValue(d.code) || hasValue(d.other_denominations) ||
    ...
    hasValue(d.typologies_text) || hasValue(d.district) || hasValue(d.local_entity);
  const hasDesc = hasValue(d.description) || hasValue(d.development) ||
    hasValue(d.spatial_description) || hasValue(d.origins) || hasValue(d.evolution);
  const hasPractice = hasValue(d.preparations) || hasValue(d.clothing) ||
    hasValue(d.instruments);
  const hasAgents = hasValue(d.agents_description) ||
    hasValue(d.transmission_mode) || hasValue(d.transformations);
            ...
            <div className="flex items-center gap-2 mb-1">
              <span
                className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                  HERITAGE_COLORS[asset.heritage_type] ?? "bg-stone-100 text-stone-700"
                }`}
              >
