"""create document_chunks_v6 for SINAI enriched embeddings
...
def upgrade() -> None:
    ...
    # ------------------------------------------------------------------
    op.create_index(
        "ix_document_chunks_v6_document_id",
        "document_chunks_v6",
        ["document_id"],
    )
    ...
    # ------------------------------------------------------------------
    op.execute(
        "ALTER TABLE document_chunks_v6 "
    ...
    # ------------------------------------------------------------------
    op.execute(
        "CREATE INDEX ix_document_chunks_v6_search_vector "
        "ON document_chunks_v6 USING GIN (search_vector)"
    )
    ...
    # ------------------------------------------------------------------
    # 8. Attach trigger to document_chunks_v6
    # ------------------------------------------------------------------
    op.execute(
        """
...
def downgrade() -> None:
    op.execute(
        "DROP TRIGGER IF EXISTS trg_update_search_vector_v6 ON document_chunks_v6"
    )
    op.execute("DROP FUNCTION IF EXISTS update_search_vector_v6()")
    op.drop_table("document_chunks_v6")
